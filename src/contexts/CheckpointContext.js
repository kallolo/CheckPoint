...
const CheckpointContextProvider = (props) => {
    ...
    photos.forEach((item, i) => {
      data.append('fotoCheckpoint', {
        ...
        uri:
          Platform.OS === 'android'
            ? item.response.uri
            : item.response.uri.replace('file://', ''),
      });
    });
  ...
  const kirimCheckpoint = async (
    ...
    // console.log(photos)
    const inputCheckpoint = await createFormData(photos, {
      idLokasi,
      keteranganCheckpoint,
      jawaban,
    });
  ...
  const getCheckpoint = async (tanggal, shift) => {
    ...
    try {
      const response = await APICheckpoint.get(
        `checkpoint/filter?shift=${shift}&tanggal=${tanggal}`,
        {headers: header},
      );
  ...
  const getDetailCheckpoint = async (tanggal, shift, by) => {
    ...
    try {
      const response = await APICheckpoint.get(
        `checkpoint/detail/${tanggal}/${shift}/${by}`,
      );
