...
const MasterLokasi = () => {
  const {stateML, refresh, getMasterLokasi, deleteMasterLokasi} = useContext(
    MasterLokasiContext,
  );
    ...
    return (
      <Swipeable
            ...
            <>
              <TouchableOpacity
                onPress={() => {
                  Alert.alert(
                    'Info',
                    `Apakah anda yakin menghapus lokasi ${data.item.namaLokasi} ?`,
                    [
                      {
                        text: 'Yakin',
                        onPress: () => deleteMasterLokasi(data.item.idLokasi),
                      },
                      {
                        text: 'Tidak',
                ...
                <View
                  ...
                  <Animated.View
                    style={{paddingHorizontal: 5, transform: [{scale}]}}>
                    <Icon name="ios-trash-outline" size={45} color="white" />
                  </Animated.View>
                </View>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => navigate('UbahLokasi', {item: data.item})}>
                <View
                  style={{
                    flex: 1,
        ...
        <List.Item
          ...
          description={
            'long : ' +
            data.item.longitudeLokasi +
            ', lat :' +
            data.item.latitudeLokasi
          }
          left={(props) => (
            <List.Icon {...props} icon="map-marker-radius" color="#007acc" />
          )}
          style={{
            borderBottomWidth: 1,
