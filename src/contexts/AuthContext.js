...
const AuthContextProvider = (props) => {
    ...
    const Login = async({username, password}) =>{
        ...
        try {
            const response = await APICheckpoint.post('/LoginSSO',{username, password});
            ...
            if(pesanLogin === "sukses"){
                ...
                dispatch({type:'loginGagal', data :  {pesan: response.data.text_msg, status : false}})
            }
        }catch(err){
            console.log(err)
        }
    ...
    const LogOut = async() =>{
        ...
        dispatch({type:'logout', data : { pesan:'berhasil logout', status:false, token:null}})
        navigate('Auth')
    }

    return (
        <AuthContext.Provider value={{ dispatch, stateAuth, viewPassword, Login, LogOut, CekToken }}>
            {props.children}
