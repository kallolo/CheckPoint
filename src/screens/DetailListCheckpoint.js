...
const DetailListCheckpoint = ({navigation}) => {
  ...
  useEffect(() => {
    getDetailCheckpoint(
      item.tanggalCheckpoint,
      item.shiftCheckpoint,
      item.createdBy,
    );
  }, []);
...
DetailListCheckpoint.navigationOptions = (screenProps) =>
  ...
  ({
    title:
      screenProps.navigation.state.params.item.detailPersonal.personalName +
      ' (Shift-' +
      screenProps.navigation.state.params.item.shiftCheckpoint +
      ', ' +
      Moment(screenProps.navigation.state.params.item.tanggalCheckpoint).format(
        'D MMM YYYY',
      ) +
      ')',
    headerStyle: {
